...
void main()
{
 ...
 int /*@  predicates{(((((x1+x2)+x4)+x6)+x7)>=1),((((x2+x3)+x4)+x7)>=1),x7<1,x7<=x1,x7<=x2,x7<=x3,x7<=x4,x7<=x5,x7<=x6,x7>=0,x7>=x1,x7>=x2,x7>=x3,x7>=x4,x7>=x5,x7>=x6} terms_tpl{x7+x1,x7+x2,x7+x3,x7+x4,x7+x5,x7+x6,x7-x1,x7-x2,x7-x3,x7-x4,x7-x5,x7-x6} @*/ x7;
 int /*@  predicates{(((((x1+x2)+x4)+x5)+x6)<=p),(((((x1+x2)+x4)+x5)+x6)>=p),p!=x6,p<1} @*/ p;
 int /*@  predicates{((((x2+x3)+x4)+x7)==q),((((x2+x3)+x4)+x7)>=q),q!=x7,q<1} @*/ q;
