...
def readHornClausesAndHints(path):
    hornVocab = Counter()
    hintVocab = Counter()
...
def main():
    ...
    train_X,train_Y=readHornClausesAndHints(path)
    X_train, X_test, y_train, y_test = train_test_split(train_X, train_Y, test_size=0.2, random_state=42)

    # checkSplitData(X_train, X_test, y_train, y_test)

    encodedPrograms_train,encodedPrograms_test,encodedHints_train,encodedHints_test=transformDatatoFeatures_tokennizer(X_train,X_test)
    ...
    history=train(encodedPrograms_train,encodedPrograms_test,encodedHints_train,encodedHints_test,y_train, y_test,batch_size,epochs)
