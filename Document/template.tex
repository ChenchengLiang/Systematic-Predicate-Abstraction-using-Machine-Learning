...
\KwResult{$T_{0}^{l},T_{1}^{l},...,T_{n}^{l}$}
 initialization: length($T_{0}^{l},T_{1}^{l},...,T_{n}^{l}$) = L\;
 Solvalbility=CEGAR($\{T_{0}^{l},T_{1}^{l},...,T_{n}^{l}\}$,HornClauses)\;
   \eIf{Solvalbility == True}{
    \While{While L>0}{
        CurrentTemplateList = $\{T_{0}^{l},T_{1}^{l},...,T_{n}^{l}\} - \{T_{k}^{l}\} (0=<k<=n)$\;
        Solvalbility=CEGAR(CurrentTemplateList,HornClauses)\;
        \eIf{Solvalbility == True}{
            RedunantTemplatesList=RedunantTemplatesList $\cup$ $T_{k}^{l}$;\
        }{
            CriticalTemplatesList=CriticalTemplatesList $\cup$ $T_{k}^{l}$;\
        }
        L- -\;
    }\;
   instructions2\;
