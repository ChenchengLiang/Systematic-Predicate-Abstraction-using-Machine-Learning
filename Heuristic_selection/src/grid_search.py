...
def main():
    ...
    for hyper_parameters in hyper_parameters_list:
        ...
        for param in parameter_list:
            ...
            if pickle == False:
                trained_model_path = train_on_graphs(benchmark_name=param.absolute_path[
                                                                    param.absolute_path.find("/benchmarks/") + len(
    ...
    with open("trained_model/" + "grid_search" + "-" + label + "-" + graph_type + "-" + benchmark + ".log",
        ...
        for result in predict_results:
            ...
            out_file.write(
                "{0:.2%}".format(max(result["best_set_threshold"], result["best_set_ranks"]) - random_guess_accuracy) +
